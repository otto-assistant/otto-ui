...
- Usage windows (`5h`, `weekly`, `monthly`) are resolved by `providers/opencode-go.js` in two steps:
  1. **Official endpoint first:** `GET https://opencode.ai/zen/go/v1/usage` (Bearer key from `auth.json`). This endpoint does not exist yet (opencode#16513 is unmerged and returns 404); it is queried first so the provider starts working automatically once OpenCode ships it.
  2. **Console dashboard fallback:** `providers/opencode-go-dashboard.js` fetches the authenticated Go dashboard (`https://opencode.ai/workspace/<id>/go`) and parses the `rollingUsage` / `weeklyUsage` / `monthlyUsage` windows (`usagePercent`, `resetInSec`, `status`) out of the SolidJS SSR hydration markup. This is the authoritative source today and matches the percentages shown on opencode.ai. Configure it with `OPENCODE_GO_WORKSPACE_ID` + `OPENCODE_GO_AUTH_COOKIE` env vars, or `~/.config/opencode/opencode-go.json` (or `~/.config/opencode/opencode-quota/opencode-go.json`) containing `workspaceId` and `authCookie`. The auth cookie comes from opencode.ai DevTools → Application → Cookies.
- The percentage from the source stays authoritative for the progress bars; dollar labels are derived from the documented limits ($12 / $30 / $60). When only an API key is configured (no dashboard credentials), the provider returns an actionable setup message instead of inaccurate numbers.
