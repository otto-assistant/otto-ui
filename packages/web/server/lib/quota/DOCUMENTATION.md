...
- Usage windows (`5h` → Rolling Usage, `weekly`, `monthly`) are resolved by `providers/opencode-go.js` from authoritative sources only:
  1. **Official endpoint:** `GET https://opencode.ai/zen/go/v1/usage` (Bearer key from `auth.json`). This endpoint does not exist yet (opencode#16513 is unmerged and returns 404); it is queried first so the provider starts working automatically once OpenCode ships it.
  2. **Console dashboard:** `providers/opencode-go-dashboard.js` fetches the authenticated Go dashboard (`https://opencode.ai/workspace/<id>/go`) and parses the `rollingUsage` / `weeklyUsage` / `monthlyUsage` windows (`usagePercent`, `resetInSec`, `status`) out of the SolidJS SSR hydration markup. This is the authoritative source today and matches the percentages shown on opencode.ai. Configure it with `OPENCODE_GO_WORKSPACE_ID` + `OPENCODE_GO_AUTH_COOKIE` env vars, or `~/.config/opencode/opencode-go.json` (or `~/.config/opencode/opencode-quota/opencode-go.json`) containing `workspaceId` and `authCookie`. The auth cookie comes from opencode.ai DevTools → Application → Cookies.
- **No local DB fallback for windows.** The local `opencode.db` `message` table only contains messages sent from this machine and would severely undercount account-wide spend (especially the monthly window). To avoid showing inaccurate data, the provider reports `configured: false` when neither the official endpoint nor the dashboard is available. This means the OpenCode Go provider only appears in the UI when the dashboard is configured.
- Percentages are shown as the primary label (e.g. `39%`) matching the OpenCode Go dashboard style, not dollar amounts.
- Per-model spend (`usage.models`) is read from the local OpenCode SQLite database (`~/.local/share/opencode/opencode.db`, `message` table) by `providers/opencode-go-usage-db.js` (`readOpenCodeModelUsage`), aggregating assistant-message `cost`/`tokens` by `modelID` for the OpenCode provider aliases. This is display-only enrichment (omitted if the DB is unavailable). The reader uses `bun:sqlite` under Bun and `better-sqlite3` under Node.
